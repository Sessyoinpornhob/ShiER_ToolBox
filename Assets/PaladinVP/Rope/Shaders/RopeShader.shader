    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                // 计算绳索的方向向量
                float3 ropeDir = normalize(_Point2.xyz - _Point1.xyz);
                float ropeLength = distance(_Point1.xyz, _Point2.xyz);
                float3 baseWorldPos = _Point1.xyz + ropeDir * (t * ropeLength);

                // 计算绳索的“右方向”（宽度方向）
                float3 upDir = float3(0, 1, 0);  // 假设世界 Y 轴为上
                float3 ropeRight = normalize(cross(upDir, ropeDir));

                // 计算局部偏移
                float3 localOffset = v.position.x * ropeRight + v.position.y * upDir;

                // 计算最终世界坐标
                float3 worldPos = baseWorldPos + localOffset;

                // 计算重力形变
                float midPointFactor = sin(t * 3.14159);
                worldPos.y -= _GravityFactor * midPointFactor;
