Shader "May100/2.5/BumpMapping"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // 修改此参数增加层数，或者把这个放在参数里。
                // float numLayers = 50.0;

                float mlod = Callod(uv);

                float layerHeight = 1.0 / _NumLayers;

                float currentLayerHeight = 0.0;

                float2 offlayerUV = viewDirTS.xy / viewDirTS.z * _HeightScale;

                float2 Stepping = offlayerUV / _NumLayers;

                float2 currentUV = uv;

                float2 AddUV = float2(0, 0);
                
                // 这个地方需要使用 tex2Dlod 指定mipmap层，因此需要自己实现mip层的计算。
                // tex2D(_HeightMap, currentUV + AddUV).r;
                float currentHeightMapValue = tex2Dlod(_HeightMap, float4(currentUV + AddUV, 0, mlod)).r;
                ...
                for (int i = 0; i < _NumLayers; i++)
                {
                    ...
                    // 
                    // tex2D(_HeightMap, currentUV + AddUV).r;
                    currentHeightMapValue = tex2Dlod(_HeightMap, float4(currentUV + AddUV, 0, mlod)).r;
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                //视差映射
                    ...
                    #ifdef _RELIEFMAP
                        offuv = SteepParallaxMapping( i.uv2.xy, viewDirTS);  //陡峭视差映射
                    #else
