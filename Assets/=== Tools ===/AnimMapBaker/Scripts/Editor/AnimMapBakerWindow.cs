...
public class AnimMapBakerWindow : EditorWindow {
    ...
    private static string _path = "VATs";
    private static string _subPath = "SubPath";
    private static SaveStrategy _strategy = SaveStrategy.AnimMap;
    ...
    [MenuItem("Tools/ShiERToolBox/VAT/AnimMapBaker")]
    public static void ShowWindow()
    {
        EditorWindow.GetWindow(typeof(AnimMapBakerWindow));
        _baker = new AnimMapBaker();
        var shaderName = GraphicsSettings.defaultRenderPipeline != null ? URPShader : BuiltInShader;
    ...
    private static Texture2D SaveAsAsset(ref BakedData data)
    {
        ...
        // solution_01 因为png无法存负值 因此用不了
        // string pngPath = Path.Combine(folderPath, data.Name + ".png");
        // SaveTextureAsPNG(animMap, pngPath);
